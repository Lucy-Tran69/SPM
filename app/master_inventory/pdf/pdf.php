...
$searchMaker = "";
   ...
   // echo json_encode ($status);die();
    if(!empty($code)){
        $zenkaku = mb_convert_kana($code, "CKV");
        $hankaku = mb_convert_kana($code,"hkV");
        $searchCd = " and (commodity.name like '%$zenkaku%' or commodity.name like '%$hankaku%') ";
    }
    if(!empty($maker)){
...
class PDF extends PDF_Japanese
{
    ...
    function Header()
    {
        ...
        if($this->count==0)
        {
            ...
            // if(strpos($this->name,'\n'))
            $this->SetX(125);
            $this->MultiCell(0,6,mb_convert_encoding(trim($this->name), 'SJIS'),0,'', false);
            // $this->Ln();
            $this->SetX(125);
            $this->Cell(0, 10,mb_convert_encoding("TEL :".$this->tel." FAX :".$this->fax, 'SJIS'),0,0,'', false);
            $this->SetFont('SJIS','',12);
            //line break
            $this->Ln();
